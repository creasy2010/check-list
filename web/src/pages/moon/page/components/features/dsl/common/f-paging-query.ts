...
export async function apply(context:IProps & {data:IDialogData}) {
  ...
  let tplInfo = getContent(context.data,actorInfo.actorName);
...
function getContent(data:IDialogData,actorName:string) {
  ...
  return {
      ...
      events: [
        {
          ...
          comment: '修改查询条件数据',
          content: `immerUtil.assign(state.request${key},payload);`,
          param: '',
        },
        {
          name: 'cleanList'+key,
          comment: '清空查询结果',
          content: `state.list${key} = [];`,
          param: '',
        },
        {
          name: 'queryResult'+key,
          comment:"",
          content: `state.list${key} = payload.list;
        state.total${key} = payload.total;
        `,
          param: '',
        },
      ],
    ...
    action: {
      methods: [
        {
          ...
          comment: `
              ...
              `,
          content: `
          this.emit(Command.${actorName}.modifyRequest${key}, param);
      //修改完直接查询;
      if(options.isQuery) {
      ...
      this.emit(Command.${actorName}.modifyRequest${key}, request);
      await this.query${key}();
        ...
        },
        {
          ...
          content: `
            if(isResetPage) { 
              this.emit(Command.${actorName}.modifyRequest${key}, {pageNum:0});
            }
          let {request${key}} =  getData().main;
      //TODO 接口缺失 
      // let {} = await api.${apiFile}.${methodName}();
      
         this.emit(Command.${actorName}.queryResult${key}, {
        total:0,
        list:[]
        } );
        `,
